...
Good commit messages help future developers (including yourself) understand
_why_ changes were made, not just what changed. When you revisit code in 6
months, "Fix student enrollment conflicts" tells you more than "Update
Enrollment model."
...
**Good:** `Fix student enrollment conflict detection`
...
```
Enable side-by-side comparison of schedule runs

Users can now compare multiple engine outputs to pick the best schedule.
Each run automatically creates a named version and links all enrollments.
...
Checklist and tasklist updates should always be committed separately from code
...
- [ ] Co-authorship footer included (if agent-generated)
